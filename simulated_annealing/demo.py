...
class SA:
    def __init__(self, func, iter=100, T0=100, Tf=0.01, alpha=0.99):
        ...
        self.T = T0              #当前温度
        self.x = [random() * 11 -5  for _ in range(iter)] #随机生成100个x的值
        self.y = [random() * 11 -5  for _ in range(iter)] #随机生成100个y的值
        self.most_best =[]
        """
    ...
    def constraint(self,x,y):
        if x < 12.1 and x > -3 and y > 4.1 and y < 5.8:
            return True
        else: return False
